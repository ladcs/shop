...
const PackTable = () => {
  ...
  useEffect(() => {
    const arrayToGetPacks = packsList.filter((pack, index, self) => self
      .findIndex(({ pack_id }) => pack_id === pack.pack_id) === index);
    const arrayToPacks = arrayToGetPacks.map(({ pack_id }) => packsList
      .filter(p => pack_id === p.pack_id));
  ...
  return (
    <TableContainer component={Paper}>
      <Table>
        ...
        <TableBody>
          { packs.map((p) => p.map((row, i) => (
            <TableRow key ={i}>
              { i === 0 ? <TableCell>{row.pack_id}</TableCell>: <TableCell></TableCell> }
              <TableCell>{ getProductName(row.product_id) }</TableCell>
              <TableCell>{ row.qty }</TableCell>
              { i === 0 ? <TableCell>{ getPricePack(p).toFixed(2)}</TableCell>: <TableCell></TableCell> }
            </TableRow>
          ))
          )}
        </TableBody>
